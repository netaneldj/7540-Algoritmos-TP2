...
def abrir_archivo_precios(ruta):
	'''Funcion que recibe una ruta de un archivo de formato csv, lo lee y devuelve una lista de tuplas,
...
def validar_fecha(lista_precios,momento):
	...
	while True:
		...
		for elemento in range(12):
			if fecha!=lista_precios[elemento][2]:
				continue
			return fecha
...
def validar_producto(produ):
	'''Funcion que recibe el diccionario de productosi,crea una lista de los posibles productos que
...
def validar_periodo(lista_precios):
	'''Funcion que pide dos fechas al usuario, verifica que no sean iguales y las devuelve'''
	while True:
		fecha_ini=validar_fecha(lista_precios,"inicial")
		fecha_fin=validar_fecha(lista_precios,"final")
...
def imprimir_inflacion_por_supermercado(lista_precios):
	'''Funcion que recibe una lista de productos con sus precios en cada fecha en cada 
	supermercado e imprime en pantalla la inflacion general de cada supermercado'''
	fecha_ini,fecha_fin=validar_periodo(lista_precios)
...
def calcular_inflacion_super(lista_precios,fecha_ini,fecha_fin):
	'''Funcion que recibe una lista de productos con sus precios en cada fecha en cada supermercado
	...
	for elemento in range(len(lista_precios)):
		if((lista_precios[elemento][2])==fecha_ini):
		...
		elif((lista_precios[elemento][2])==fecha_fin):
			if(lista_precios[elemento][0]=="1"):
...
def inflacion_por_producto(dicc_super,dicc_produ,lista_precios):
	'''Funcion que recibe un diccionario de supermercados,uno de productos y una lista de productos con precios en
	...
	producto=elegir_producto_en_lista_posibles(dicc_super,dicc_produ,lista_precios)
...
def precio_ini_fin(sup,lista_precios,indice_prod_dicc_produ,fecha_ini,fecha_fin):
	...
	for j in range(len(lista_precios)):
		if (lista_precios[j][0]==sup):
			if(int(lista_precios[j][1])==indice_prod_dicc_produ):
				if((lista_precios[j][2])==fecha_ini):
					precio_ini_sup=float(lista_precios[j][3])
				elif((lista_precios[j][2])==fecha_fin):
...
def inflacion_general_promedio(dicc_super,dicc_produ,lista_precios):
	'''Funcion que recibe un diccionario de supermercados y otro de productos y una lista de precios
	...
	for x in range(len(lista_precios)):
		if((lista_precios[x][2])==fecha_ini):
			suma_precio_ini+=float(lista_precios[x][3])
		elif((lista_precios[x][2])==fecha_fin):
...
def mejor_precio_para_un_producto(dicc_super,dicc_produ,lista_precios):
	'''Funcion pide al usuario un producto y una fecha, se fija en la lista de precios cual supermercado
	tiene ese producto mas barato en esa fecha y lo imprime en pantalla'''
	supermercado=0
	producto=elegir_producto_en_lista_posibles(dicc_super,dicc_produ,lista_precios)
	for i in range(len(dicc_produ)):
		if (producto==dicc_produ[str(i+1)]):
			...
			break
	fecha=validar_fecha(lista_precios,"")
	for x in range(len(lista_precios)):
		if (int(lista_precios[x][1])==indice_prod_dicc_produ):
			if((lista_precios[x][2])==fecha):
...
def elegir_producto_en_lista_posibles(dicc_super,dicc_produ,lista_precios):
...
def ciclo_eleccion(dicc_super,dicc_produ,lista_precios,opcion):
	...
	if(opcion==INFLACION_POR_SUPERMERCADO):
			imprimir_inflacion_por_supermercado(lista_precios)
...
def main():
	'''Funcion principal del programa, crea las lista y diccionarios correspondientes,
